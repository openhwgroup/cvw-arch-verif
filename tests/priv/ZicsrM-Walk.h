...
1:  csrrs t6, mstatus, t1    # set all bits
	...
	bnez t0, 1b         # repeat until all bits are walked
	li t0, 0x1800
	csrrs t6, mstatus, t0    # mstatus.MPP=3
	slli t0, t0, 21 # shift to mstatus.UXL for RV64; drops off end and is ignored for RV32
	csrrs t6, mstatus, t0    # mstatus.UXL=3
	slli t0, t0, 2 # shift to mstatus.SXL for RV64
	csrrs t6, mstatus, t0    # mstatus.SXL=3
...
1:  csrrs t6, mtvec, t1    # set all bits
	...
	bnez t0, 1b         # repeat until all bits are walked
	li t0, 0x03
	csrrs t6, mtvec, t0 # xtvec.MODE=3
...
1:  csrrs t6, menvcfg, t1    # set all bits
	...
	bnez t0, 1b         # repeat until all bits are walked
	li t0, 0x30
	csrrs t6, menvcfg, t0 # xenvcfg.CBIE=3
...
1:  csrrs t6, stvec, t1    # set all bits
	...
	bnez t0, 1b         # repeat until all bits are walked
	li t0, 0x03
	csrrs t6, stvec, t0 # xtvec.MODE=3
...
1:  csrrs t6, senvcfg, t1    # set all bits
	...
	bnez t0, 1b         # repeat until all bits are walked
	li t0, 0x30
	csrrs t6, senvcfg, t0 # xenvcfg.CBIE=3
...
// Testing walking zeros and ones for CSR stimecmp
	csrr s0, stimecmp	# save CSR
	li t1, -1           # all 1s
	li t0, 1            # 1 in lsb
	1: csrrc t6, stimecmp, t1    # clear all bits
	csrrs t6, stimecmp, t0    # set walking 1
	slli t0, t0, 1      # walk the 1
	bnez t0, 1b         # repeat until all bits are walked
	li t0, 1            # 1 in lsb
1:  csrrs t6, stimecmp, t1    # set all bits
	csrrc t6, stimecmp, t0    # clear walking 1
	slli t0, t0, 1      # walk the 1
	bnez t0, 1b         # repeat until all bits are walked
	csrrw t6, stimecmp, s0    # restore CSR

// Testing walking zeros and ones for CSR stimecmph
	csrr s0, stimecmph	# save CSR
	li t1, -1           # all 1s
	li t0, 1            # 1 in lsb
	1: csrrc t6, stimecmph, t1    # clear all bits
	csrrs t6, stimecmph, t0    # set walking 1
	slli t0, t0, 1      # walk the 1
	bnez t0, 1b         # repeat until all bits are walked
	li t0, 1            # 1 in lsb
1:  csrrs t6, stimecmph, t1    # set all bits
	csrrc t6, stimecmph, t0    # clear walking 1
	slli t0, t0, 1      # walk the 1
	bnez t0, 1b         # repeat until all bits are walked
	csrrw t6, stimecmph, s0    # restore CSR
