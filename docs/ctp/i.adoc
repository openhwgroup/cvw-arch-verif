...
give the target address.|{beq/bne/blt/bge/bltu/bgeu}/cr_simple_edges_offset, ***Issue 717
|2.5.2|BEQ and BNE take the branch
if registers _rs1_ and _rs2_ are equal or unequal respectively.|{beq/bne}/cr_simple_edges_offset ***Issue717
|2.5.2|BLT and
BLTU take the branch if _rs1_ is less than _rs2_, using signed and
unsigned comparison respectively. |{blt/bltu}/cr_simple_edges_offset
|2.5.2|BGE and BGEU take the branch if _rs1_
is greater than or equal to _rs2_, using signed and unsigned comparison
respectively.|{bge/bgeu}/cr_simple_edges_offset
...
as are LB and LBU for 8-bit values.|lb/cp_rd_edge_byte, lbu/cp_rd_edge_byte, lh/cp_rd_edge_half, lhu/cp_rd_edge_half, lw/cp_rd_edge_word, lwu/cp_rd_edge_word ***Issue717
