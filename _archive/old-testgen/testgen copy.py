...
def insertTemplate(name, xlen=0, extension="", is_custom=False) -> str:
    ...
    )  # TODO: Make this configurable for some tests (e.g. Zimop)
...
def writeSIGUPD_F(rd):
    ...
    l = (
        l + f"RVTEST_SIGUPD_F(x{sigReg}, f{rd}, x{tempReg})\n"
...
def loadFloatReg(reg, val, xlen, flen):  # *** eventually load from constant table instead
    ...
    if precision > xlen:  # precision = 64, xlen = 32
        lines = (
            lines
            + f"LI(x{tempReg}, 0x{formatstrFP.format(val)[10:18]}) # load x{tempReg} with 32 MSBs {formatstrFP.format(val)}\n"
        )
        lines = (
            lines + f"{storeop} x{tempReg}, 0(x2) # store x{tempReg} (0x{formatstrFP.format(val)[10:18]}) in memory\n"
        )
        lines = (
            lines
            + f"LI(x{tempReg}, 0x{formatstrFP.format(val)[2:10]}) # load x{tempReg} with 32 LSBs of {formatstrFP.format(val)}\n"
        )
        lines = (
            lines
            + f"{storeop} x{tempReg}, 4(x2) # store x{tempReg} (0x{formatstrFP.format(val)[2:10]}) in memory 4 bytes after x3\n"
        )
        lines = lines + f"{loadop} f{reg}, 0(x2) # load {formatstrFP.format(val)} from memory into f{reg}\n"
    else:
        lines = (
            lines
            + f"LI(x{tempReg}, {formatstrFP.format(val)}) # load x{tempReg} with value {formatstrFP.format(val)}\n"
        )
...
def handleSignaturePointerConflict(lines, rs1, rs2, rd, rs3=None):
    ...
    oldSigReg = sigReg
    while (
        sigReg == rs1 or sigReg == rs2 or sigReg == rd or sigReg == rs3 or sigReg == 2
    ):  # avoid conflict with the test registers and stackpointer since it is used for loads
        sigReg = randint(1, 31)
...
def getSigSpace(xlen, flen, sigupd_count, sigupd_countF, test):
    ...
    if sigupd_countF > 0:
        if flen > xlen:
            mult = flen // xlen
            hasXregHaz = (
                1 if test in F2Xtype or test in fTOrtype else 2
            )  # when test is not to Xtype hazards cause sigupd_count missalignment skips
            signatureWords = (
                hasXregHaz * sigupd_count + (sigupd_countF * mult)
            )  # multiply be reg ratio to get correct amount of Xlen/32 4byte blocks for footer and double sigupd_count for alignment in hazard cases
        else:
            signatureWords = (
                sigupd_count + sigupd_countF
            )  # all Sigupd, when Xlen is larger than Flen and SIGUPD_F macro will adjust alignment up to XLEN
    signatureWords += (
        (4 // (xlen // 32)) * (((signatureWords * (xlen // 32)) * 4) // 2016)
    )  # add additional space for offset overflow adjustment offset reset at offset of 2016 so calculate offset amount from sigwords then multiply by addisional sigwords needed to cover the overflow realignment
...
def writeCovVector(desc, rs1, rs2, rd, rs1val, rs2val, immval, rdval, test, xlen, rs3=None, rs3val=None, frm=False):
    ...
    if test in rtype:
        ...
        lines = writeTest(
            ...
            xlen,
            False,
            test + " x" + str(rd) + ", x" + str(rs1) + ", x" + str(rs2) + " # perform operation\n",
        )
    elif test in i1type:
        lines = lines + "LI(x" + str(rs1) + ", " + formatstr.format(rs1val) + ") # initialize rs1\n"
        ...
        if not frm:
            lines = writeTest(
                ...
                xlen,
                True,
                test + " f" + str(rd) + ", f" + str(rs1) + ", f" + str(rs2) + " # perform operation\n",
            )
        else:
        ...
        if not frm:
            lines = writeTest(
                ...
                + ", f"
                + str(rs1)
                + " # perform operation second time to make sure flags don't change\n",
            )
        else:
        ...
        if test in csrtype:
            ...
            lines = writeTest(
                lines, rd, xlen, False, f"{test} x{rd}, mscratch, {unsignedImm5(immval)} # perform operation\n"
            )
            ...
            if test == "c.lwsp" or test == "c.flwsp":
                ...
                if test == "c.fldsp":  ##special case for c.fldsp in rv32 have to load double in two parts
                    ...
                    if flen > xlen:  # flen = 6
                        lines = (
                            lines
                            + f"LI(x{temp1}, 0x{formatstrFP.format(rdval)[2:10]}) # load x{temp1} with 32 MSBs of {formatstrFP.format(rdval)}\n"
                        )
                        lines = (
                            lines
                            + f"LI(x{temp2}, 0x{formatstrFP.format(rdval)[10:18]}) # load x{temp2} with 32 LSBs {formatstrFP.format(rdval)}\n"
                        )
                        lines = (
                            lines
                            + f"{storeop} x{temp1}, {int(ZextImm6(immval)) * mul}(sp) # store x{temp1} (0x{formatstrFP.format(rdval)[2:10]}) in memory\n"
                        )
                        lines = lines + "addi sp, sp, 4 # move address up by 4\n"
                        lines = (
                            lines
                            + f"{storeop} x{temp2}, {int(ZextImm6(immval)) * mul}(sp) # store x{temp2} (0x{formatstrFP.format(rdval)[10:18]}) after 4 bytes in memory\n"
                        )
                        lines = lines + "addi sp, sp, -4 # move back to scratch\n"
                    else:
                        lines = (
                            lines
                            + f"LI(x{temp1}, {formatstrFP.format(rdval)}) # load x{temp1} with value {formatstrFP.format(rdval)}\n"
                        )
                        lines = (
                            lines
                            + f"{storeop} x{temp1}, {int(ZextImm6(immval)) * mul}(sp) # store {formatstrFP.format(rdval)} in memory\n"
                        )
                    lines = writeTest(
                        lines, rd, xlen, True, f"{test} f{rd}, {int(ZextImm6(immval)) * mul}(sp) # perform operation\n"
                    )
                else:
        ...
        if test == "c.addi16sp":
            ...
            lines = (
                ...
                + "sp"
                + ", -"
                + str(int(ZextImm6(immval)) * mul)
                + " # sub immediate from rs1 to counter offset\n"
            )
            lines = (
                lines
                + storeop
                ...
                + "("
                + "sp"
                + ")   # store value to put something in memory\n"
            )
            if test == "c.flwsp":
                lines = writeTest(
                    lines,
            ...
            if test in ["c.li", "c.addi", "c.addiw"]:  # Add tests with signed Imm in the list
                lines = writeTest(
                    lines, rd, xlen, False, test + " x" + str(rd) + ", " + signedImm6(immval) + " # perform operation\n"
                )
            elif test == "c.lui":
                lines = writeTest(
                    lines, rd, xlen, False, test + " x" + str(rd) + ", " + SextImm6(immval) + " # perform operation\n"
                )
            else:
                lines = writeTest(
                    lines, rd, xlen, False, test + " x" + str(rd) + ", " + ZextImm6(immval) + " # perform operation\n"
                )
        ...
        if test in ["c.add"]:
            ...
            lines = writeTest(
                lines, rd, xlen, False, test + " x" + str(rd) + ", x" + str(rs2) + " # perform operation\n"
            )
        elif test in ["c.mv"]:
            if rs2 == 0:
                rs2 = 11
            ...
            if test in "c.jalr":
                lines = (
                    ...
                    + "LI(x1"
                    + ", "
                    + formatstr.format(rdval)
                    + ") # initialize rd (x1) to a random value that should get changed\n"
                )
        ...
        lines = writeTest(
            ...
            xlen,
            False,
            test + " x" + str(rd) + ", sp, " + str(int(unsignedImm8(immval)) * 4) + " # perform operation\n",
        )
        ...
        lines = writeTest(
            ...
            xlen,
            False,
            test + " x" + str(rd) + ", x" + str(rs1) + ", " + shiftImm(immval, xlen) + " # perform operation\n",
        )
        ...
        lines = writeTest(
            ...
            xlen,
            False,
            test + " x" + str(rd) + ", x" + str(rs1) + ", " + shiftImm(immval, 32) + " # perform operation\n",
        )
    elif test in itype:
        lines = lines + "LI(x" + str(rs1) + ", " + formatstr.format(rs1val) + ") # initialize rs1\n"
        lines = writeTest(
            ...
            xlen,
            False,
            test + " x" + str(rd) + ", x" + str(rs1) + ", " + signedImm12(immval) + " # perform operation\n",
        )
    elif test in ibtype:
        lines = lines + "LI(x" + str(rs1) + ", " + formatstr.format(rs1val) + ") # initialize rs1\n"
        lines = writeTest(
            ...
            + str(rs1)
            + ", "
            + ibtype_unsignedImm(xlen, immval)
            + " # perform operation\n",
        )
        ...
        if rs1 != 0:
            ...
            if immval == -2048:
                ...
                lines = (
                    ...
                    + ", x"
                    + str(rs1)
                    + ", 1 # increment rs1 to bump it by a total of 2048 to compensate for -2048\n"
                )
            else:
                lines = (
                    lines
            ...
            lines = writeTest(
                ...
                xlen,
                False,
                test + " x" + str(rd) + ", " + signedImm12(immval) + "(x" + str(rs1) + ") # perform operation\n",
            )
    elif test in cltype:
        while rs1 == rs2:
        ...
        if test in ["c.lw", "c.ld"]:
            ...
            lines = (
                ...
                + str(rs1)
                + ", -"
                + str(int(unsignedImm5(immval)) * mul)
                + " # sub immediate from rs1 to counter offset\n"
            )
            lines = (
                lines
            ...
            if test == "c.flw":
                ...
                lines = (
                    ...
                    + str(rs2)
                    + ", "
                    + formatstr.format(rs2val)
                    + ") # load immediate value into integer register\n"
                )
                lines = (
                    lines
                    + "sw x"
                ...
                while temp2 in [rs1, rs2, temp1]:
                    temp2 = randomNonconflictingReg(test)
                lines = (
                    lines + "addi x" + str(rs1) + ", x" + str(rs1) + ", " + str(int(unsignedImm5(immval)) * mul) + "\n"
                )
                if flen > xlen:  # flen = 6
                    rs2val = (rs2val << 32) | (rs1val)
                    lines = (
                        lines
                        + f"LI(x{temp1}, 0x{formatstrFP.format(rs2val)[2:10]}) # load x{temp1} with 32 MSBs of {formatstrFP.format(rs2val)}\n"
                    )
                    lines = (
                        lines
                        + f"LI(x{temp2}, 0x{formatstrFP.format(rs2val)[10:18]}) # load x{temp2} with 32 LSBs {formatstrFP.format(rs2val)}\n"
                    )
                    lines = (
                        lines
                        + f"{storeop} x{temp1}, {int(unsignedImm5(immval)) * mul}(x{rs1}) # store x{temp1} (0x{formatstrFP.format(rs2val)[2:10]}) in memory\n"
                    )
                    lines = lines + f"addi x{rs1}, x{rs1}, 4 # move address up by 4\n"
                    lines = (
                        lines
                        + f"{storeop} x{temp2}, {int(unsignedImm5(immval)) * mul}(x{rs1}) # store x{temp2} (0x{formatstrFP.format(rs2val)[10:18]}) after 4 bytes in memory\n"
                    )
                    lines = lines + f"addi x{rs1}, x{rs1}, -4 # move back to scratch\n"
                else:
                    lines = (
                        lines
                        + f"LI(x{temp1}, {formatstrFP.format(rs2val)}) # load x{temp1} with value {formatstrFP.format(rs2val)}\n"
                    )
                    lines = (
                        lines
                        + f"{storeop} x{temp1}, {int(unsignedImm5(immval)) * mul}(x{rs1}) # store {formatstrFP.format(rs2val)} in memory\n"
                    )
                lines = writeTest(
                    lines,
                    rd,
                    xlen,
                    True,
                    f"{test} f{rd}, {int(unsignedImm5(immval)) * mul}(x{rs1}) # perform operation\n",
                )
        ...
        lines = (
            ...
            + str(rs1)
            + ", -"
            + str(int(unsignedImm1(immval)) * mul)
            + " # sub immediate from rs1 to counter offset\n"
        )
        lines = (
            lines
        ...
        if not ((test == "c.fsd") and (flen > xlen)):
            lines = (
                lines + "LI(x" + str(rs2) + ", " + formatstr.format(rs2val) + ") # initialize rs2 with random value\n"
            )
        ...
        if test in ["c.fsw", "c.fsd"]:
            if (test == "c.fsd") and (flen > xlen):
                ...
                lines = (
                    ...
                    + str(sigReg)
                    + ")"
                    + " # clearing the random value store at scratch\n"
                )
        lines = (
            lines
            + "addi x"
        ...
        storeline = (
            test
            + (" f" if test in ["c.fsw", "c.fsd"] else " x")
            + str(rs2)
            + ", "
            + str(int(unsignedImm5(immval)) * mul)
            + "(x"
        ...
        lines = (
            lines + "addi x" + str(sigReg) + ", x" + str(sigReg) + ", " + str(int(unsignedImm5(immval)) * mul) + " \n"
        )
        if test == "c.fsd":
        ...
        if rs1 != 0:
            ...
            if immval == -2048:
                lines = (
                    ...
                    + str(sigReg)
                    + ", "
                    + makeImm(-1 * (-2047), 12, True)
                    + " \n"
                )
                lines = (
                    lines + "addi x" + str(sigReg) + ", x" + str(sigReg) + ", " + makeImm(-1 * (-1), 12, True) + " \n"
                )
            else:
                lines = (
                    lines + "addi x" + str(sigReg) + ", x" + str(sigReg) + ", " + makeImm(-1 * immval, 12, True) + " \n"
                )
        ...
        if test in ["c.sd", "c.fsd", "c.sdsp", "c.fsdsp"]:
            ...
            WIDTH = "REGWIDTH"
        lines = (
            lines + "addi x" + str(sigReg) + ", x" + str(sigReg) + ", " + str(WIDTH) + " # Incrementing base register\n"
        )
        sigupd_count += 1
        ...
        lines = (
            lines
            + f"addi x{rs1}, x{rs1}, {signedImm12(-immval, immOffset=True)} # add immediate to lower part of rs1\n"
        )
        jumpline = f"{test} x{rd}, x{rs1}, {signedImm12(immval, immOffset=True)} # perform operation\n"
        lines = writeJumpTest(lines, rd, rs1, rs2, xlen, jumpline)
    elif test in utype:  # ["lui", "auipc"]
        lines = writeTest(
            lines, rd, xlen, False, test + " x" + str(rd) + ", " + unsignedImm20(immval) + " # perform operation\n"
        )
        ...
        if flen > xlen:  # flen = 64, xlen = 32
            lines = (
                lines
                + f"LI(x{tempreg1}, 0x{formatstrFP.format(rs2val)[2:10]}) # load x3 with 32 LSBs of {formatstrFP.format(rs2val)}\n"
            )
            lines = (
                lines
                + f"LI(x{tempreg2}, 0x{formatstrFP.format(rs2val)[10:18]}) # load x3 with 32 MSBs {formatstrFP.format(rs2val)}\n"
            )
            lines = (
                lines
                + f"{storeop} x{tempreg1}, {signedImm12(immval)}(x{rs1}) # store x3 (0x{formatstrFP.format(rs2val)[2:10]}) in memory\n"
            )
            lines = lines + f"addi x{rs1}, x{rs1}, 4 # move address up by 4\n"
            lines = (
                lines
                + f"{storeop} x{tempreg2}, {signedImm12(immval)}(x{rs1}) # store x4 (0x{formatstrFP.format(rs2val)[10:18]}) in memory 4 bytes after x3\n"
            )
            lines = lines + f"addi x{rs1}, x{rs1}, -4 # move back to scratch\n"
        else:
            lines = (
                lines
                + f"LI(x{tempreg1}, {formatstrFP.format(rs2val)}) # load x3 with value {formatstrFP.format(rs2val)}\n"
            )
            lines = (
                lines
                + f"{storeop} x{tempreg1}, {signedImm12(immval)}(x{rs1}) # store {formatstrFP.format(rs2val)} in memory\n"
            )
        ...
        if not frm:
            ...
            lines = writeTest(
                lines, rd, xlen, False, test + " x" + str(rd) + ", f" + str(rs1) + rm + " # perform operation\n"
            )
        else:
        ...
        lines = writeTest(
            ...
            xlen,
            False,
            test + " x" + str(rd) + ", f" + str(rs1) + ", f" + str(rs2) + " # perform operation\n",
        )
        lines = lines + writeFcsrSIG()  # write fcsr to signature register
        ...
        lines = writeTest(
            ...
            xlen,
            False,
            test + " x" + str(rd) + ", x" + str(rs1) + ", " + str(immval % 11) + " # perform operation\n",
        )
    elif test in lrtype:
        lines = lines + "LI(x" + str(rs2) + ", " + formatstr.format(rs2val) + ") # initialize rs2\n"
        ...
        lines = writeTest(
            ...
            xlen,
            False,
            test + " x" + str(rd) + ", x" + str(rs2) + ", (x" + str(rs1) + ") # perform operation\n",
        )
    elif test in amotype:
        lines = lines + "LI(x" + str(rs1) + ", " + formatstr.format(rs1val) + ") # initialize rs1\n"
        lines = lines + "LI(x" + str(rs2) + ", " + formatstr.format(rs2val) + ") # initialize rs2\n"
...
def writeSingleInstructionSequence(
    desc, testlist, regconfiglist, rdlist, rs1list, rs2list, rs3list, immvalslist, commentlist, xlen
):
    ...
    for testindex, test in enumerate(testlist):
        ...
        if insMap[instype].get("loadstore") == "load":
            lines += (
                ...
                + " "
                + regconfig[0]
                + str(registerArray[0][testindex])
                + ", "
                + signedImm12(immvalslist[testindex])
                + "("
                + regconfig[1]
                + str(registerArray[1][testindex])
                + ")"
                + " # "
                + commentlist[testindex]
                + "\n"
            )
            ...
            for regindex, reg in enumerate(regconfiglist[testindex]):
                match reg:
                    case "a":
                        reg = "x"
                        lines += (
                            "," * (lines[-1 * len(test) :] != test)
                            + " ("
                            + reg
                            + str(registerArray[regindex][testindex])
                            + ")"
                        )
                    case "x" | "f":
                        lines += (
...
def writeHazardVector(
    ...
    if testb in jalrtype:
        ...
        lines += f"LI(x{rs3a}, 1) \n"
        immvalb = 0
        lines += writeSingleInstructionSequence(
            desc, [testa], [regconfig2], [rda], [rs1a], [rs2a], [rs3a], [immvala], ["perform first operation"], xlen
        )
        lines += writeSingleInstructionSequence(
            desc,
            [testb],
        ...
        if haz_type != "war":
            ...
            if haz_type == "nohaz":
                lines = (
                    lines + "LI(x" + str(rda) + ", " + formatstr.format(immvalb) + ") # initialize random imm value\n"
                )
            else:
                if (
                    testb in ["fsw", "fsh"] and haz_type == "raw"
                ):  # have to load value into freg to have read after write be same reg
                    hazStoreOp = "fsw"
                    hazRegType = "f"
                    tempReg = 3
                    tempReg2 = 4
                    ...
                    lines = (
                        lines
                        + f"LI(x{tempReg}, {formatstrFP.format(immvalb)}) # load x{tempReg} with value {formatstrFP.format(immvalb)}\n"
                    )
                    lines = lines + f"sw x{tempReg}, 0(x{tempReg2}) # store {formatstrFP.format(immvalb)} in memory\n"
                    lines = (
                        lines
                        + f"flw f{rs2b}, 0(x{tempReg2}) # load {formatstrFP.format(immvalb)} from memory into f{rs2b}\n"
                    )
                elif (
                    testb == "fsd" and haz_type == "raw"
                ):  # have to load value into freg to have read after write be same reg
                    hazStoreOp = "fsw"
                    hazRegType = "f"
                    tempReg = 3
                    ...
                    lines = (
                        lines
                        + f"LI(x{tempReg}, 0x{formatstrFP.format(immvalb)[10:18]}) # load x{tempReg} with 32 MSBs {formatstrFP.format(immvalb)}\n"
                    )
                    lines = (
                        lines
                        + f"sw x{tempReg}, 0(x{tempReg2}) # store x{tempReg} (0x{formatstrFP.format(immvalb)[10:18]}) in memory\n"
                    )
                    lines = (
                        lines
                        + f"LI(x{tempReg}, 0x{formatstrFP.format(immvalb)[2:10]}) # load x{tempReg} with 32 LSBs of {formatstrFP.format(immvalb)}\n"
                    )
                    lines = (
                        lines
                        + f"sw x{tempReg}, 4(x{tempReg2}) # store x{tempReg} (0x{formatstrFP.format(immvalb)[2:10]}) in memory 4 bytes after x3\n"
                    )
                    lines = (
                        lines
                        + f"fld f{rs2b}, 0(x{tempReg2}) # load {formatstrFP.format(immvalb)} from memory into f{rs2b}\n"
                    )
                else:
                    lines = (
                        lines
                        ...
                        + str(rs2b)
                        + ", "
                        + formatstr.format(immvalb)
                        + ") # initialize random imm value\n"
                    )
        ...
        if test in amotype:
            ...
            lines = lines + "LI(x" + str(rs1b) + ", " + formatstr.format(immvala) + ") # initialize rs2\n"
        if (testa in floattypes and testa not in fTOrtype) or (
            testb in floattypes and testb not in fTOrtype
        ):  # clear flags for floating point operations before performing
    ...
    for r in rng:
        ...
        if test == "jal":
            if r >= 6:  # Can only fit signature logic if jump is greater than 32 bytes (r+1=6)
                ...
                lines = (
                    ...
                    + "_"
                    + test
                    + " # jump to aligned address to stress immediate\n"
                )
            else:
                lines = (
                    lines
            ...
            if r >= 6:  # Can only fit signature logic if jump is greater than 32 bytes (r+1=6)
                ...
                lines = lines + writeSIGUPD(rs1)
                lines = (
                    lines + test + " f" + str(r + 1) + "_" + test + " # jump to aligned address to stress immediate\n"
                )
            else:
                lines = (
                    lines + test + " f" + str(r + 1) + "_" + test + " # jump to aligned address to stress immediate\n"
        ...
        if test == "jal":
            ...
            if r == 12:  # temporary fix for bug in compressed branches
                ...
                lines = (
                    lines + test + " b" + str(r - 1) + "_" + test + " # jump to aligned address to stress immediate\n"
                )
                if r >= 6:
                    lines = lines + f"c.li x{rs1}, 0" + "\n"
                    lines = lines + writeSIGUPD(1)  # checking if return address is correct for c.jal
...
def make_imm_edges_jalr(test, xlen):
    ...
    for immval in IMMEDIATE_EDGES.imm_12bit:
        ...
        lines = (
            ...
            + ", "
            + signedImm12(immval)
            + " # jump to assigned address to stress immediate\n"
...
def make_offset(test, xlen):
    ...
    if test in btype:
        ...
        lines = lines + test + " x0, x 0, 1b # backward branch\n"
...
def write_tests(coverpoints, test, xlen=None, vlen=None, sew=None, vlmax=None, vl=1, lmul=1):
    ...
    for coverpoint in coverpoints:
        ...
        #   make_fs3_edges(test, xlen, fedgesQ)
            ...
            make_cr_rs1_imm_edges(
                test, xlen, IMMEDIATE_EDGES.imm_uimm if xlen == 64 else IMMEDIATE_EDGES.imm_uimmw
            )  # more unsigned immediates for RV64
...
floattypes = (
    ...
    + PX2Ftype
    + zcdtype
)  # TODO: these types aren't necessary anymore, Hamza remove them
...
insMap = {
    ...
    "citype": {
        "instructions": ["c.lui", "c.li", "c.addi", "c.addi16sp", "c.addiw", "c.lwsp", "c.ldsp", "c.flwsp", "c.fldsp"],
        "regconfig": "xi__",
        "compressed": 1,
        "implicitxreg": "_x__",
    },
    ...
    "cjtype": {"instructions": cjtype, "regconfig": "l___"},
    "compressed": 1,
...
def main() -> None:
    ...
    for xlen in xlens:
        edges_imm_c = (
            IMMEDIATE_EDGES.imm_32_c if xlen == 32 else IMMEDIATE_EDGES.imm_64_c
        )  # 32-bit or 64-bit immediate edges for compressed shifts
        ...
        for E_ext in [False]:  # for testing only ***
            ...
            for extension in extensions:
                ...
                for test in coverpoints.keys():
                    ...
                    f.write(insertTemplate("testgen_header.S", xlen, extension))
                    ...
                    # print footer
                    signatureWords = getSigSpace(
                        xlen, flen, sigupd_count, sigupd_countF, test
                    )  # figure out how many words are needed for signature
                    f.write(insertTemplate("testgen_footer.S", xlen, extension))
