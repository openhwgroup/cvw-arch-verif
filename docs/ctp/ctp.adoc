...
**** V listed in Table A.6 for 20.  Seems wrong
*** Zicclsm
...
=== Unspecified, Reserved, Bitfield, Strict, and Illegal Behaviors

Some RISC-V behaviors are UNSPECIFIED, which means an implementation can do anything. For example, the behavior of a reserved instruction is UNSPECIFIED. These behaviors are not tested because there is no wrong answer.

WARL, WLRL, WPRI


*** definitions, which isn't tested
