...
Some RISC-V behaviors are UNSPECIFIED, which means an implementation can do anything. These behaviors are not tested because there is no wrong answer. <<t-behavior-normative-statements>> summarizes the normative rules regarding reserved behaviors.
...
<<t-bitfield-normative-statements>> summarizes the normative rules regarding CSR bitfields.
