...
// Testing walking zeros and ones for CSR menvcfgh
	csrr s0, menvcfgh	# save CSR
	li t1, -1           # all 1s
	li t0, 1            # 1 in lsb
	1: csrrc t6, menvcfgh, t1    # clear all bits
	csrrs t6, menvcfgh, t0    # set walking 1
	slli t0, t0, 1      # walk the 1
	bnez t0, 1b         # repeat until all bits are walked
	li t0, 1            # 1 in lsb
1:  csrrs t6, menvcfgh, t1    # set all bits
	csrrc t6, menvcfgh, t0    # clear walking 1
	slli t0, t0, 1      # walk the 1
	bnez t0, 1b         # repeat until all bits are walked
	csrrw t6, menvcfgh, s0    # restore CSR

// Testing walking zeros and ones for CSR 0x312
	csrr s0, 0x312	# save CSR
	li t1, -1           # all 1s
	li t0, 1            # 1 in lsb
	1: csrrc t6, 0x312, t1    # clear all bits
	csrrs t6, 0x312, t0    # set walking 1
	slli t0, t0, 1      # walk the 1
	bnez t0, 1b         # repeat until all bits are walked
	li t0, 1            # 1 in lsb
1:  csrrs t6, 0x312, t1    # set all bits
	csrrc t6, 0x312, t0    # clear walking 1
	slli t0, t0, 1      # walk the 1
	bnez t0, 1b         # repeat until all bits are walked
	csrrw t6, 0x312, s0    # restore CSR
#endif
