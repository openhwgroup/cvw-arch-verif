...
# vxrm tests
vxrmList = {"rod": "0x6", "rdn": "0x4", "rne": "0x2", "rnu": "0x0"}  # vcsr[2:1] -> 11 , 10, 01, 00
...
def genRandomVectorLS():
    ...
    num_words_either_side = (
        int(maxELEN / 64) * 2 * 2 * maxVLEN
    )  # 2 times max vlen elements on either side of pointer (sewMAX = 64)
...
def genVMaskEdges():
    ...
    while regenerate:  # prevent overlapping with other mask edges
        ...
        for i in range(3, int(math.log2(maxVLEN))):  # getting all powers of 2: 8 through maxVLEN
            ...
            if (random_mask_bottom_vlen_bits == 0) or (
                random_mask_bottom_vlen_bits % 2 == 1
            ):  # if any of them overlap with a mask edge
                regenerate = True
...
def genVsEdges(test, sew, emul):
    def convert(val, bitwidth):
        if (sew == 64) or (eew == 64):
            return [f"0x{(val >> (eew * i)) & 0xFFFFFFFFFFFFFFFF:016x}" for i in range((bitwidth + (eew - 1)) // eew)]
        else:
            return [f"0x{(val >> (eew * i)) & 0xFFFFFFFF:08x}" for i in range((bitwidth + (eew - 1)) // eew)]
    ...
    while (r := randint(3, 2 ** (eew - 1) - 3)) in set(v_register_edges.values()):
        pass
...
def writeSIGUPD_F(fd):
    ...
    writeLine(
        f"RVTEST_SIGUPD_F(x{sigReg}, f{fd}, x{tempReg})", f"# store f{fd} and x{tempReg} (fcsr) in signature"
...
def writeSIGUPD_V(vd, sew, avl=1, sig_lmul=None, load_testline=None, sig_whole_register_store=False):
    ...
    if "SEWMIN" in str(sew):
        if sig_whole_register_store:
            writeLine(
                f"vsetvli x{tempReg}, x0, SEWSIZE, m{sig_lmul}, ta, ma",
                f"# change lmul to {sig_lmul} and set vl to vlmax to store register(s) (offgroup)",
            )
        writeLine(
            f"RVTEST_SIGUPD_V_SEWMIN(x{sigReg}, x{tempReg}, {avl}, v{vd})",
            f"# stores v{vd} (sew = SEWMIN, AVL = {avl}) in signature with base (x{sigReg}) and helper (x{tempReg}) register",
        )
    else:
        if sig_whole_register_store:
            writeLine(
                f"vsetvli x{tempReg}, x0, e{sew}, m{sig_lmul}, ta, ma",
                f"# change lmul to {sig_lmul} and set vl to vlmax to store register(s) (offgroup)",
            )
        writeLine(
            f"RVTEST_SIGUPD_V(x{sigReg}, x{tempReg}, {avl}, {sew},  v{vd})",
            f"# stores v{vd} (sew = {sew}, AVL = {avl}) in signature with base (x{sigReg}) and helper (x{tempReg}) register",
        )
...
def loadVecReg(instruction, register_argument_name: str, vector_register_data, sew, lmul, *scalar_registers_used):
    ...
    if instruction in whole_register_move:
        ...
        if instruction not in whole_register_stores:  # stores do not encode an eew
            register_eew = getInstructionEEW(
                instruction
            )  # for whole register loads and stores the eew is hardcoded for all registers but does not affect the register group size (completely independent of the vtype csr)
        register_emul = getInstructionSegments(instruction)
        preloaded_value_emul = lmul * register_eew / sew
        load_unique_vtype = (
            register % lmul != 0
            or preloaded_value_emul < 1 / 8
            or preloaded_value_emul > 8
            or register % math.ceil(preloaded_value_emul) != 0
        )
        register_emul = 1
    ...
    if register_eew > 64 or register_eew < 8:
        raise ValueError(f"Register EEW violates constraints: register_eew = {register_eew}")
    ...
    if load_unique_vtype:
        ...
        writeLine(
            f"vsetvli x0, x{avlReg}, e{register_eew}, m{math.ceil(register_emul)}, ta, ma", "# set lmul to 1 for load"
    ...
    if load_vls_random_edge:
        register_val_pointer = "vector_ls_random_base"
    ...
    if register_value is not None:
        ...
        if register_val_pointer == "vs_edge_zero_emul8":
            ...
            segment_instruction_phrase = (
                f"seg{register_segments}" if register_segments is not None and register_segments != 1 else ""
            )
            writeLine(
                f"vl{segment_instruction_phrase}e{register_eew}.v v{register}, (x{tempReg})",
                f"# Load desired value from memory into v{register}",
            )
    ...
    if (
        register_argument_name == "vs2" and instruction in vector_ls_ins
    ):  # make sure elements in vs2 are within VLMAX and sew aligned
        vtypeReg = 1
        while vtypeReg in scalar_registers_used:
            vtypeReg = randint(1, 31)
        scalar_registers_used.append(vtypeReg)
        ...
        if eew < xlen:  # make sure the number is positive since it will be 0 extended to XLEN
            element_positive_reg = 15
            while element_positive_reg in scalar_registers_used:
                element_positive_reg = randint(1, 31)
            scalar_registers_used.append(element_positive_reg)
            writeLine(
                f"li x{element_positive_reg}, {element_positive}",
                "#  make sure the number is positive since it will be 0 extended to XLEN",
            )
            writeLine(f"vand.vx v{register}, v{register}, x{element_positive_reg}", "#  ")
        writeLine(
            f"vrem.vx v{register}, v{register}, x{vlmaxReg}", "# ensure all values are within (-2*vlmax, 2*vlmax)"
        )
...
def loadFloatReg(sew, register_argument_name: str, floating_point_register_data, *scalar_registers_used):
    ...
    if precision > xlen:  # precision = 64, xlen = 32
        writeLine(
            f"LI(x{memoryReg}, 0x{formatstrFP.format(register_value)[10:18]})",
            f"# load x{memoryReg} with 32 MSBs {formatstrFP.format(register_value)}",
        )
        writeLine(
            f"{storeop} x{memoryReg}, 0(x{scratchReg})",
            f"# store x{memoryReg} (0x{formatstrFP.format(register_value)[10:18]}) in memory",
        )
        writeLine(
            f"LI(x{memoryReg}, 0x{formatstrFP.format(register_value)[2:10]})",
            f"# load x{memoryReg} with 32 LSBs of {formatstrFP.format(register_value)}",
        )
        writeLine(
            f"{storeop} x{memoryReg}, 4(x{scratchReg})",
            f"# store x{memoryReg} (0x{formatstrFP.format(register_value)[2:10]}) in memory 4 bytes after x{scratchReg}",
        )
        writeLine(
            f"{loadop} f{register}, 0(x{scratchReg})",
            f"# load {formatstrFP.format(register_value)} from memory into f{register}",
        )
    else:
        writeLine(
            f"LI(x{memoryReg}, {formatstrFP.format(register_value)})",
            f"# load x{memoryReg} with value {formatstrFP.format(register_value)}",
        )
        writeLine(
            f"{storeop} x{memoryReg}, 0(x{scratchReg})", f"# store {formatstrFP.format(register_value)} in memory"
        )
        writeLine(
            f"{loadop} f{register}, 0(x{scratchReg})",
            f"# load {formatstrFP.format(register_value)} from memory into f{register}",
...
def handleSignaturePointerConflict(*registers):
    ...
    if sigReg != oldSigReg:
        writeLine(
            "mv x" + str(sigReg) + ", x" + str(oldSigReg),
            "# switch signature pointer register to avoid conflict with test",
        )
...
def getSigSpace(xlen, flen):
    ...
    if sigupd_countF > 0:
        if flen > xlen:
            mult = flen // xlen
            signatureWords = (
                sigupd_count + (sigupd_countF * (mult * 2 - 1))
            )  # multiply be reg ratio to get correct amount of Xlen/32 4byte blocks for footer and double the count for alignment (4 and 8 need 16 byts)
        else:
            signatureWords = (
                sigupd_count + sigupd_countF
            )  # all Sigupd, no need to adjust since Xlen is equal to or larger than Flen and SIGUPD_F macro will adjust alignment up to XLEN
...
def writeVecTest(
    ...
    *scalar_registers_used,
    ...
    if priv:
        ...
        writeLine(
            f"vsetivli x0, 1, SEWSIZE, m{sig_lmul}, tu, mu",
            f"# re-initialize vl = 1, LMUL = {sig_lmul}, SEW = SEWMIN for signature",
        )
    ...
    if (test in vd_widen_ins) or (test in wvsins):
        writeSIGUPD_V(
            ...
            avl=vl,
            sig_lmul=sig_lmul,
            load_testline=load_testline,
            sig_whole_register_store=sig_whole_register_store,
        )  # EEW of vd = 2 * SEW for widening
...
def getInstructionArguments(instruction):
    ...
    # test writing
    if instruction in vvvmtype:
        instruction_arguments = ["vd", "vs2", "vs1", "vm"]
    elif instruction in vvvtype:
        instruction_arguments = ["vd", "vs2", "vs1"]
    elif instruction in vvvmrtype:
        instruction_arguments = ["vd", "vs1", "vs2", "vm"]
    elif instruction in vvxmtype:
        instruction_arguments = ["vd", "vs2", "rs1", "vm"]
    elif instruction in vvfmtype:
        instruction_arguments = ["vd", "vs2", "fs1", "vm"]
    elif instruction in vxvmtype:
        instruction_arguments = ["vd", "rs1", "vs2", "vm"]
    elif instruction in vfvmtype:
        instruction_arguments = ["vd", "fs1", "vs2", "vm"]
    elif instruction in vvimtype:
        instruction_arguments = ["vd", "vs2", "imm", "vm"]
    elif instruction in vvivtype:
        instruction_arguments = ["vd", "vs2", "imm", "v0"]
    elif instruction in vvvvtype:
        instruction_arguments = ["vd", "vs2", "vs1", "v0"]
    elif instruction in vvxvtype:
        instruction_arguments = ["vd", "vs2", "rs1", "v0"]
    elif instruction in vvfvtype:
        instruction_arguments = ["vd", "vs2", "fs1", "v0"]
    elif instruction in xvmtype:
        instruction_arguments = ["rd", "vs2", "vm"]
    elif instruction in xvtype:
        instruction_arguments = ["rd", "vs2"]
    elif instruction in fvtype:
        instruction_arguments = ["fd", "vs2"]
    elif instruction in vvmtype:
        instruction_arguments = ["vd", "vs2", "vm"]
    elif instruction in vmtype:
        instruction_arguments = ["vd", "vm"]
    elif instruction in vxtype:
        instruction_arguments = [
            "vd",
            "rs1",
        ]
    elif instruction in vftype:
        instruction_arguments = ["vd", "fs1"]
    elif instruction in vvrtype:
        instruction_arguments = ["vd", "vs1", "vm"]
    elif instruction in vvvxtype:
        instruction_arguments = ["vd", "vs2", "vm"]
    elif instruction in vitype:
        instruction_arguments = ["vd", "imm", "vm"]
    elif instruction in type_vsx:
        instruction_arguments = ["vs3", "rs1"]
    elif instruction in type_vsxm:
        instruction_arguments = ["vs3", "rs1", "vm"]
    elif instruction in type_vsxxm:
        instruction_arguments = ["vs3", "rs1", "rs2", "vm"]
    elif instruction in type_vsxvm:
        instruction_arguments = ["vs3", "rs1", "vs2", "vm"]
    elif instruction in type_vx:
        instruction_arguments = ["vd", "rs1"]
    elif instruction in type_vxm:
        instruction_arguments = ["vd", "rs1", "vm"]
    elif instruction in type_vxvm:
        instruction_arguments = ["vd", "rs1", "vs2", "vm"]
    elif instruction in type_vxxm:
        instruction_arguments = ["vd", "rs1", "rs2", "vm"]
    else:
...
def writeTest(
    ...
    for argument in instruction_arguments:
        if argument == "vm":
            ...
            scalar_registers_used = loadVecReg(
                instruction, argument, vector_register_data, sew, lmul, *scalar_registers_used
            )
    ...
    if (
        vector_register_data["vd"]["reg_type"] == "mask"
        or vector_register_data["vd"]["reg_type"] == "scalar"
        or (
            instruction in whole_register_ls
            and (lmul != getInstructionSegments(instruction) or sew != getInstructionEEW(instruction))
        )
    ):
        sig_lmul = 1
    ...
    if instruction in vector_stores:  # for stores we reload the value saved to memory to check against signature
        load_instruction = getLoadEquivalentInstruction(instruction, sew)
        load_instruction_data = randomizeVectorInstructionData(
            load_instruction,
            sew,
            None,
            None,
        ...
        [load_vector_register_data, load_scalar_register_data, _load_floating_point_register_data, _load_imm_val] = (
    ...
    if (
        instruction in vector_ls_ins
        and instruction not in indexed_ls_ins
        and getInstructionEEW(instruction) is not None
    ):
...
def getLoadEquivalentInstruction(instruction, sew):
...
def prepBaseV(sew, lmul, vl=1, vstart=0, ta=0, ma=0, *scalar_registers_used):
    ...
    if vl == "random":
        ...
        writeLine(
            f"vsetvli x{tempReg2}, x0, e{sew}, m{lmulflag}{taflag}{maflag}",
            f"# Set vl = VLMAX, where x{vlmaxReg} = VLMAX",
        )
    else:
    ...
    if register is None:  # if the register is a vector register
        if register_type == "v":
            ...
            if register_data["reg_type"] == "scalar" or register_data["reg_type"] == "mask" or emul < 1:
                emul = 1
            register = emul * randint(
                0, int(reg_count / emul) - (segments)
            )  # only register numbers of multiples of LMUL(EMUL) are allowed, segments must not go past reg 31
...
def getInstructionRegisterOverlapConstraints(instruction, sew, lmul):
    ...
    if instruction in wvvins:
        no_overlap = [["vd_bottom", "vs2"], ["vd_bottom", "vs1"]]
    elif instruction in vupgatherins:
        no_overlap = [["vd", "vs2"], ["vd", "vs1"]]
    elif instruction in vmlogicalins:
        no_overlap = [["vd", "vs2"]]
    elif instruction in wvxins:
        no_overlap = [["vd_bottom", "vs2"]]
    elif instruction in fwvfins:
        no_overlap = [["vd_bottom", "vs2"]]
    elif instruction in mv_ins:
        no_overlap = [["vd", "vs2"], ["vd", "vs1"]]  # mv_ins can never be masked
    elif instruction in fmvvins:
        no_overlap = [["vd", "vs2"], ["vd", "vs1"]]  # fmvvins can be masked
    ...
    elif instruction in seg_vv_load:
        no_overlap = [["vd", "vs2"]]

    if instruction in vector_ls_ins:
        no_overlap = addOverlap(no_overlap, [["rs1", "rs2"]])
    ...
    if ls_indexed_vs2_eew is not None and not isinstance(sew, str):
        if ls_indexed_vs2_eew > sew:
            if (
                lmul * ls_indexed_vs2_eew / sew <= 1
            ):  # if vs2 emul is <= 1 then all overlap is bottom thus we do not need the overlap constraint
                pass
            else:
...
def addOverlap(instruction_overlap_constraints, additional_no_overlap):
    no_overlap = []
    if additional_no_overlap is not None:
        no_overlap = no_overlap + additional_no_overlap
    if instruction_overlap_constraints is not None:
        no_overlap = no_overlap + instruction_overlap_constraints
...
def randomizeVectorInstructionData(
    instruction, sew, test_count, suite="base", lmul=1, additional_no_overlap=None, **preset_variables
):
    preset_variables.update(getVectorEmulMultipliers(instruction))

    instruction_overlap_constraints = getInstructionRegisterOverlapConstraints(instruction, sew, lmul)
    no_overlap = addOverlap(instruction_overlap_constraints, additional_no_overlap)
    ...
    if eew is not None:  # if emul is greater than 1 use it for the size multiplier
        if instruction in ls_no_eew_ins:
            pass  # these use implied lmul as the number of registers to load instead of size_multiplier
        elif instruction in whole_register_ls:
            pass  # do not reference lmul nor sew
        elif instruction in indexed_ls_ins:
    ...
    if no_overlap == []:
        ...
        vector_register_data["vs3"] = randomizeRegister(
            instruction, eew, "vs3", vreg_count, vector_register_preset_data, lmul
        )
        vector_register_data["vd"] = randomizeRegister(
            instruction, eew, "vd", vreg_count, vector_register_preset_data, lmul
        )
        vector_register_data["vs1"] = randomizeRegister(
            instruction, eew, "vs1", vreg_count, vector_register_preset_data, lmul
        )
        vector_register_data["vs2"] = randomizeRegister(
            instruction, eew, "vs2", vreg_count, vector_register_preset_data, lmul
        )
        ...
        scalar_register_data["rs1"] = randomizeRegister(
            instruction, eew, "rs1", xreg_count, scalar_register_preset_data
        )
        scalar_register_data["rs2"] = randomizeRegister(
            instruction, eew, "rs2", xreg_count, scalar_register_preset_data
        )

        floating_point_register_data["fd"] = randomizeRegister(
            instruction, eew, "fd", freg_count, floating_point_register_preset_data
        )
        floating_point_register_data["fs1"] = randomizeRegister(
            instruction, eew, "fs1", freg_count, floating_point_register_preset_data
        )
    ...
    while register_overlap:
        ...
        if randomization_count >= max_randomization_count:
            raise ValueError(
                f'No Overlap constraint "{no_overlap}" cannot be met for instruction "{instruction}" with sew "{sew}", eew "{eew}" and lmul "{lmul}" after {max_randomization_count} attempts'
            )
...
def getInstructionNfields(instruction):
    if instruction == "vmv1r.v":
        return 1
    elif instruction == "vmv2r.v":
        return 2
    elif instruction == "vmv3r.v":
        return 3
    elif instruction == "vmv4r.v":
        return 4
    elif instruction == "vmv5r.v":
        return 5
    elif instruction == "vmv6r.v":
        return 6
    elif instruction == "vmv7r.v":
        return 7
    elif instruction == "vmv8r.v":
        return 8
    else:
        return None
...
def getBaseLmul(instruction, sew):
    if instruction in eew8_ins and 8 / sew > 1:
        return sew / 8
    elif instruction in eew16_ins and 16 / sew > 1:
        return sew / 16
    elif instruction in eew32_ins and 32 / sew > 1:
        return sew / 32
    elif instruction in eew64_ins and 64 / sew > 1:
        return sew / 64
    elif instruction in whole_register_move:
